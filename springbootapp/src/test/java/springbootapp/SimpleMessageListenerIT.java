...
public class SimpleMessageListenerIT {
	...
	static LocalStackContainer localStack = new LocalStackContainer(
			DockerImageName.parse("localstack/localstack:0.10.0")).withServices(S3, SQS).withEnv("DEFAULT_REGION",
					"eu-central-1");
	...
	static class AwsTestConfig {
		...
		public AmazonS3 amazonS3() {
			return AmazonS3ClientBuilder.standard().withCredentials(localStack.getDefaultCredentialsProvider())
					.withEndpointConfiguration(localStack.getEndpointConfiguration(S3)).build();
		}

		@Bean
		public AmazonSQSAsync amazonSQS() {
			return AmazonSQSAsyncClientBuilder.standard().withCredentials(localStack.getDefaultCredentialsProvider())
					.withEndpointConfiguration(localStack.getEndpointConfiguration(SQS)).build();
		}
	}
	...
	public void testMessageShouldBeUploadedToS3OnceConsumed() {
		...
		given().ignoreException(AmazonS3Exception.class).await().atMost(5, SECONDS)
